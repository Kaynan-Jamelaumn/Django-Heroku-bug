...
class PwdResetForm(PasswordResetForm):
    ...
    def clean_email(self):
        ...
        if not u:
            raise forms.ValidationError(
                'Infelizmente não conseguimos encontrar este endereço de e-mail')
        return email
...
class UserEditForm(forms.ModelForm):
    ...
    user_name = forms.CharField(
        label='Firstname', min_length=4, max_length=50, widget=forms.TextInput(
            attrs={'class': 'form-control mb-3', 'placeholder': 'Usuário', 'id': 'form-firstname', 'readonly': 'readonly'}))

    first_name = forms.CharField(
        label='Username', min_length=4, max_length=50, widget=forms.TextInput(
            attrs={'class': 'form-control mb-3', 'placeholder': 'Primeiro Nome', 'id': 'form-lastname'}))
